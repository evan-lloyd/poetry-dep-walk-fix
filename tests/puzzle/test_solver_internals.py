...
def tm(solver_package_info: SolverPackageInfo) -> dict[str, str]:
    return {
        key: str(value) for key, value in solver_package_info.transitive_marker.items()
    }
...
def test_merge_packages_from_override_multiple_deps(package: ProjectPackage) -> None:
    ...
    merge_packages_from_override(
        packages,
        {a: SolverPackageInfo(0, {"main"}, {"main": AnyMarker()})},
        {
            package: {
