...
def test_installer_distinguishes_locked_packages_with_local_version_by_source(
    ...
    if lock_version == "2.1":
        ...
        lock_data["package"][0]["marker"] = "sys_platform == 'darwin'"
        lock_data["package"][1]["groups"] = ["main"]
        lock_data["package"][1]["marker"] = "sys_platform != 'darwin'"
    locker.locked(True)
    locker.mock_lock_data(lock_data)
    installer = Installer(
        NullIO(),
...
def test_installer_distinguishes_locked_packages_with_same_version_by_source(
    ...
    if lock_version == "2.1":
        lock_data["package"][0]["groups"] = ["main"]
        lock_data["package"][0]["marker"] = "platform_machine != 'aarch64'"
        lock_data["package"][1]["groups"] = ["main"]
        lock_data["package"][1]["marker"] = "platform_machine == 'aarch64'"
    locker.locked(True)
    locker.mock_lock_data(lock_data)
    installer = Installer(
        NullIO(),
...
def test_explicit_source_dependency_with_direct_origin_dependency(
    ...
    if lock_version == "2.1":
        ...
        lock_data["package"][0]["marker"] = "sys_platform != 'darwin'"
        lock_data["package"][1]["marker"] = "sys_platform == 'darwin'"
        lock_data["package"][2]["marker"] = "sys_platform != 'darwin'"
    locker.locked(True)
    locker.mock_lock_data(lock_data)
    installer = Installer(
        NullIO(),
