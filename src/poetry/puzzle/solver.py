...
class Solver:
    ...
    def _solve_in_compatibility_mode(
        ...
        for solver_info in packages.values():
            for group, marker in solver_info.transitive_marker.items():
                if (
                    ...
                    solver_info.transitive_marker[group] = AnyMarker()
...
def aggregate_package_nodes(
    ...
    return package, SolverPackageInfo(depth, groups, {})
    ...
    while has_incomplete_markers:
        ...
        for depth in range(max_depth + 1):
            for package in packages_by_depth[depth]:
                solver_info = packages[package]
                transitive_marker: dict[str, BaseMarker] = {
                    group: EmptyMarker() for group in solver_info.groups
                }
                for parent, m in markers[package].items():
                    parent_solver_info = packages[parent]
                    if parent_solver_info.groups:
                        if parent_solver_info.groups != set(
                            parent_solver_info.transitive_marker
                        ):
                            # there is a cycle -> we need one more iteration
                            has_incomplete_markers = True
                            continue
                        for group in parent_solver_info.groups:
                            transitive_marker[group] = transitive_marker[group].union(
                                parent_solver_info.transitive_marker[group].intersect(m)
                            )
                    else:
                        for group in solver_info.groups:
                            transitive_marker[group] = transitive_marker[group].union(m)
                solver_info.transitive_marker = transitive_marker


def merge_packages_from_override(
    packages: dict[Package, SolverPackageInfo],
    new_packages: dict[Package, SolverPackageInfo],
    ...
    for new_package, new_package_info in new_packages.items():
        if package_info := packages.get(new_package):
            ...
            for group, marker in new_package_info.transitive_marker.items():
                package_info.transitive_marker[group] = (
                    package_info.transitive_marker.get(
                        group, EmptyMarker()
            ...
            for group, marker in new_package_info.transitive_marker.items():
                new_package_info.transitive_marker[group] = override_marker.intersect(
                    marker
                )
